...
export default function ProjectDetail() {
  ...
  // const isAdmin = !!(user && project && (user.user_id === project.user.user_id || user.isAdmin));
    ...
    () => {
      const loadProject = async () => {
        try {
          ...
          dispatch({ type: ProjectActionType.LOAD_PROJECT_ACTION, payload: res.data });
        } catch (err) {
          console.error(err);
          // history.replace('/');
        }
    ...
    <div className={s.wrapper}>
      {project && (
        <div className={cx('content', s.inner)}>
          ...
          <div>
            id автора: {project.user_id}
          </div>
          <div>
            Дата создания: {project.created_at}
          </div>
          <div>
            Дата обновления: {project.updated_at}
          </div>
        </div>
      )}
    </div>
  ...
  //             project={project}
  //             pages={pages}
  //             canEdit={user !== null && (user.user_id === project.user.user_id) || (projectUsers && projectUsers.users.filter((u: any) => u.user_id === user?.user_id) !== -1)}
